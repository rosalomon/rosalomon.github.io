document.addEventListener('DOMContentLoaded', function() {
    ...
    function calculateValues() {
        ...
        // Hämta individuella avkastningsvärden
        const individualOverallReturnRates = document.querySelectorAll('.overall-individual-return-rate');
        const individualStockReturnRates = document.querySelectorAll('.stock-individual-return-rate');

        // Flaggor för att avgöra om vi använder individuella avkastningsvärden
        const useIndividualOverallReturns = individualOverallReturnRates.length === years;
        const useIndividualStockReturns = individualStockReturnRates.length === years;
        ...
        for (let i = 0; i < years * 12; i++) {
            ...
            // Hämta årsindividuella avkastningsvärden om de används
            let currentMonthlyStockReturnRate = monthlyStockReturnRate;
            if (useIndividualStockReturns) {
                const yearIndex = Math.floor(i / 12);
                const yearlyStockReturnRate = parseFloat(individualStockReturnRates[yearIndex].value) / 100;
                currentMonthlyStockReturnRate = (1 + yearlyStockReturnRate) ** (1 / 12) - 1;
            }

            // Uppdatera totalFutureValueRent med den månatliga investeringen och avkastningen
            if (monthlyInvestment > 0) {
                totalFutureValueRent = totalFutureValueRent * (1 + currentMonthlyStockReturnRate) + monthlyInvestment;
            } else {
                totalFutureValueRent *= (1 + currentMonthlyStockReturnRate);
            }
